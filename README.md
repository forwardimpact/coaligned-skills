...
| **coaligned-setup** | Bootstrap the Co-Aligned instruction architecture in a repository. Use when a repo has no layered agent instructions yet, when adopting the Co-Aligned standard, or when wiring `npx coaligned` into the checks so instruction layers, jobs, and invariants stay enforced. |
